...
//   console.log("Processing payment.authorized - ATTEMPTING AUTO-CAPTURE");
...
//     // 🔥 AUTO-CAPTURE THE PAYMENT IMMEDIATELY
...
//         paymentEntity.id,
//         paymentEntity.amount, // Amount in paise
//         'INR'
//       );

//       console.log("✅ PAYMENT AUTO-CAPTURED SUCCESSFULLY!");
//       console.log("Captured Payment ID:", capturedPayment.id);
...
//         status: 'captured',
//         method: capturedPayment.method || 'unknown',
//         capturedAt: new Date(),
//         updatedAt: new Date()
//       };
...
//       console.log("✅ Order updated with captured payment:", order._id);
//       logger.info('Payment auto-captured via webhook', {
//         orderId: order._id,
//         paymentId: capturedPayment.id,
//         amount: capturedPayment.amount / 100,
//         userEmail: order.userEmail
...
//     } catch (captureError) {
//       console.error("❌ AUTO-CAPTURE FAILED:", captureError.message);
      
//       // Still update order with authorized status
//       order.paymentInfo = {
//         paymentId: paymentEntity.id,
//         amount: paymentEntity.amount / 100,
//         status: 'authorized',
...
//       logger.error('Auto-capture failed in webhook', {
//         orderId: order._id,
//         paymentId: paymentEntity.id,
...
//   } catch (error) {
//     console.error("Error processing payment.authorized:", error);
//     logger.error('Error in payment.authorized handler', {
//       orderId,
//       error: error.message
//     });
//     throw error;
...
// Initialize Razorpay for capturing payments
const razorpayInstance = new Razorpay({
  key_id: process.env.RAZORPAY_KEY_ID,
  key_secret: process.env.RAZORPAY_KEY_SECRET,
});
...
// Handle payment authorized - AUTO CAPTURE IMMEDIATELY
async function handlePaymentAuthorized(eventData) {
  const paymentEntity = eventData.payment.entity;
  const orderId = paymentEntity.order_id;
  ...
  console.log("Payment ID:", paymentEntity.id);
  console.log("Order ID:", orderId);
  console.log("Amount:", paymentEntity.amount / 100);
  ...
  try {
    ...
    try {
      ...
      console.log("✅✅✅ PAYMENT AUTO-CAPTURED SUCCESSFULLY! ✅✅✅");
      console.log("Captured Payment ID:", capturedPayment.id);
      console.log("Captured Status:", capturedPayment.status);
      console.log("Captured Amount:", capturedPayment.amount / 100);
      ...
      if (order.status === 'Cancelled' && order.cancelledBy === 'system') {
        ...
        order.cancelledBy = null;
        order.cancelledAt = null;
        console.log("Order reactivated from cancelled state");
      }

      await order.save();

      console.log("✅ Order updated successfully with captured payment");
      logger.info('Payment auto-captured successfully via webhook', {
        orderId: order._id,
        paymentId: capturedPayment.id,
        amount: capturedPayment.amount / 100,
        userEmail: order.userEmail
      ...
      console.error("❌ AUTO-CAPTURE FAILED:", captureError.message);
      console.error("Capture Error Details:", JSON.stringify(captureError, null, 2));
      
      // Still update order with authorized status (manual capture needed)
      order.paymentInfo = {
        paymentId: paymentEntity.id,
        amount: paymentEntity.amount / 100,
        status: 'authorized',
      ...
      logger.error('Auto-capture failed in webhook - manual capture required', {
        orderId: order._id,
        paymentId: paymentEntity.id,
        error: captureError.message
      });
